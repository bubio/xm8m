...
MODEL = ROOT / "media_machine.json"
# The Markdown table lives with the private development docs, which are absent in public checkouts.
OUTPUT = REPO / "Documents/development/RetroAchievements/48_RA媒体状態機械遷移表.generated.md"
CPP_OUTPUT = REPO / "Source/RA/ra_media_operation_table.generated.h"
...
def render(model, matrix, by_id):
    lines = ["# RA媒体状態機械 遷移表・図（生成物）", "",
             ...
             "定義は本体リポジトリの`scripts/ra_media_machine/media_machine.json`、生成は同`generate_media_machine.py`。", "",
             "## 1. 全二次元表", "",
...
def render_cpp(model, matrix):
    ...
    lines = ["// Generated from scripts/ra_media_machine/media_machine.json. Do not edit.",
...
def main():
    ...
    if args.self_test:
        self_test(model)
    outputs = [(CPP_OUTPUT, render_cpp(model, matrix))]
    ...
    print(f"PASS: {len(matrix)} cells classified; {len(by_id)} transitions; "
          f"{len(model['scenarios'])} abstract traces; generated outputs match: "
          + ", ".join(path.name for path, _ in outputs))
